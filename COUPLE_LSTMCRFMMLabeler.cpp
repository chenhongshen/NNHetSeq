...
#include "COUPLE_LSTMCRFMMLabeler.h"
...
void Labeler::random_select_train(vector<Example>& c1_Exams,vector<Example>& c2_Exams,vector<pair<int,vector<Example> > >&type_Exams){
  ...
  vector<Example> cur_batch_exams;
...
//  }
  ...
  for (int iter = 0; iter < m_options.maxIter; ++iter) {
    ...
    random_select_train(c1_trainExamples,c2_trainExamples,type_trainExamples);
    ...
    for (int updateIter = 0; updateIter < inputSize; updateIter++) {
      ...
      int cur_type=sub_type_exmas.first;
      vector<Example>& subExamples=sub_type_exmas.second;

      int curUpdateIter = iter * inputSize + updateIter;
      dtype cost = m_classifier.process(subExamples, curUpdateIter, cur_type);
   ...
   if (c1_devNum > 0 && c2_devNum > 0) {
      ...
      for (int idx = 0; idx < c1_devExamples.size(); idx++) {
        ...
        predict(c1_devExamples[idx].m_features, result_labels, c1_devInsts[idx].words, 1);
      ...
      if (c1_testNum > 0) {
        ...
        for (int idx = 0; idx < c1_testExamples.size(); idx++) {
          vector<string> result_labels;
          predict(c1_testExamples[idx].m_features, result_labels, c1_testInsts[idx].words, 1);
        ...
        std::cout<< std::endl << "corpus1 test:" << std::endl;
      ...
      if (c2_devNum > 0) {
        ...
        for (int idx = 0; idx < c2_devExamples.size(); idx++) {
          vector<string> result_labels;
          predict(c2_devExamples[idx].m_features, result_labels, c2_devInsts[idx].words, 2);
      ...
      std::cout<< std::endl << "corpus2 dev:" << std::endl;
      c2_metric_dev.print();
      ...
      if (c2_testNum > 0) {
        ...
        for (int idx = 0; idx < c2_testExamples.size(); idx++) {
          vector<string> result_labels;
          predict(c2_testExamples[idx].m_features, result_labels, c2_testInsts[idx].words, 2);
...
int Labeler::predict(const vector<Feature>& features, vector<string>& outputs, const vector<string>& words, const int corpus_type) {
  assert(features.size() == words.size());
  vector<int> labelIdx, label2Idx;
  m_classifier.predict(features, labelIdx, corpus_type);
  outputs.clear();
...
void Labeler::test(const string& testFile, const string& outputFile, const string& modelFile, const int corpus_type) {
  ...
  metric_test.reset();
  clock_t start, end;
  double duration_ori,duration_sec;
  for (int idx = 0; idx < testExamples.size(); idx++) {
    vector<string> result_labels;
    predict(testExamples[idx].m_features, result_labels, testInsts[idx].words, corpus_type);
    if (m_options.seg) {
  ...
  duration_ori = (double)(end - start);
  duration_sec = duration_ori / CLOCKS_PER_SEC;
  std::cout <<"cost time:"<<duration_ori<<"\t"<<duration_sec<<std::endl;
...
int main(int argc, char* argv[]) {
  ...
  if (bTrain) {
    tagger.train(c1_trainFile, c1_devFile, c1_testFile,
        c2_trainFile, c2_devFile, c2_testFile,
        modelFile, optionFile, wordEmbFile, charEmbFile);
  } else {
    tagger.test(c1_testFile, outputFile, modelFile, 1);
    //tagger.test(c2_testFile, outputFile, modelFile, 2);
  }
