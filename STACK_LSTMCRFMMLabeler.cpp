...
#include "STACK_LSTMCRFMMLabeler.h"
...
void Labeler::base_convert2Example(const Instance* pInstance, Example& exam) {
  exam.clear();
  const vector<string> &labels = pInstance->labels;
  int curInstSize = labels.size();
  for (int i = 0; i < curInstSize; ++i) {
...
void Labeler::base_initialExamples(const vector<Instance>& vecInsts, vector<Example>& vecExams) {
  int numInstance;
  for (numInstance = 0; numInstance < vecInsts.size(); numInstance++) {
    const Instance *pInstance = &vecInsts[numInstance];
    Example curExam;
    base_convert2Example(pInstance, curExam);
    vecExams.push_back(curExam);
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile,
  ...
  vector<Example> base_trainExamples, base_devExamples, base_testExamples;
  vector<int> base_otherInstNums(otherInsts.size());
  vector<vector<Example> > base_otherExamples(otherInsts.size());

  base_initialExamples(trainInsts, base_trainExamples);
  base_initialExamples(devInsts, base_devExamples);
  base_initialExamples(testInsts, base_testExamples);
  for (int idx = 0; idx < otherInsts.size(); idx++) {
  ...
  for (int iter = 0; iter < m_options.maxIter; ++iter) {
    ...
    for (int updateIter = 0; updateIter < batchBlock; updateIter++) {
      ...
      for (int idy = start_pos; idy < end_pos; idy++) {
        ...
        base_subExamples.push_back(base_trainExamples[indexes[idy]]);
      }

      int curUpdateIter = iter * batchBlock + updateIter;
      dtype cost = m_classifier.process(subExamples, base_subExamples, curUpdateIter);
    ...
    if (devNum > 0) {
      ...
      for (int idx = 0; idx < devExamples.size(); idx++) {
        ...
        predict(devExamples[idx].m_features, base_devExamples[idx].m_features, result_labels, devInsts[idx].words);
      ...
      if (testNum > 0) {
        ...
        for (int idx = 0; idx < testExamples.size(); idx++) {
          vector<string> result_labels;
          predict(testExamples[idx].m_features, base_testExamples[idx].m_features, result_labels, testInsts[idx].words);
      ...
      for (int idx = 0; idx < otherExamples.size(); idx++) {
        ...
        for (int idy = 0; idy < otherExamples[idx].size(); idy++) {
          vector<string> result_labels;
          predict(otherExamples[idx][idy].m_features, base_otherExamples[idx][idy].m_features,result_labels, otherInsts[idx][idy].words);
...
int Labeler::predict(const vector<Feature>& features, const vector<Feature>& base_features, vector<string>& outputs, const vector<string>& words) {
  assert(features.size() == words.size());
  assert(base_features.size() == words.size());
  vector<int> labelIdx, label2Idx;
  m_classifier.predict(features, base_features, labelIdx);
  outputs.clear();
...
int main(int argc, char* argv[]) {
  ...
  Labeler tagger;
  if (bTrain) {
    tagger.train(trainFile, devFile, testFile, modelFile, optionFile, wordEmbFile, charEmbFile, basemodelFile);
  } else {
