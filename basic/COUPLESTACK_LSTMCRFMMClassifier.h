/*
 * COUPLESTACK_LSTMCRFMMClassifier.h
 *
...
class COUPLESTACK_LSTMCRFMMClassifier {
public:
  COUPLESTACK_LSTMCRFMMClassifier() {
    _dropOut = 0.5;
  }
  ~COUPLESTACK_LSTMCRFMMClassifier() {
...
public:
  ...
  inline dtype process(const vector<Example>& examples, const vector<Example>& base_examples,int iter, int corpus_type) {
    _eval.reset();
  ...
  void predict(const vector<Feature>& features, const vector<Feature>& base_features,vector<int>& results, int corpus_type) {
    int _labelSize=(1==corpus_type?_c1_labelSize:_c2_labelSize);
    UniLayer<xpu>& _olayer_linear=(1==corpus_type?_c1_olayer_linear:_c2_olayer_linear);
    MMCRFLoss<xpu>& _crf_layer=(1==corpus_type?_c1_crf_layer:_c2_crf_layer);
  ...
  dtype computeScore(const Example& example, const Example& base_example, const int corpus_type) {
    int _labelSize=(1==corpus_type?_c1_labelSize:_c2_labelSize);
    UniLayer<xpu>& _olayer_linear=(1==corpus_type?_c1_olayer_linear:_c2_olayer_linear);
    MMCRFLoss<xpu>& _crf_layer=(1==corpus_type?_c1_crf_layer:_c2_crf_layer);
  ...
  void updateParams(dtype nnRegular, dtype adaAlpha, dtype adaEps, const int corpus_type) {
    UniLayer<xpu>& _olayer_linear=(1==corpus_type?_c1_olayer_linear:_c2_olayer_linear);
    MMCRFLoss<xpu>& _crf_layer=(1==corpus_type?_c1_crf_layer:_c2_crf_layer);
 ...
 void loadBaseModel(LStream &inf) {
  ...
  void checkgrad(const vector<Example>& examples, const vector<Example>& base_examples,Tensor<xpu, 2, dtype> Wd, Tensor<xpu, 2, dtype> gradWd, const string& mark, int iter, const int corpus_type) {
    ...
    for (int i = 0; i < examples.size(); i++) {
      ...
      lossAdd += computeScore(oneExam, oneExam_base, corpus_type);
    }
    ...
    for (int i = 0; i < examples.size(); i++) {
      Example oneExam = examples[i];
      Example oneExam_base = base_examples[i];
      lossPlus += computeScore(oneExam, oneExam_base, corpus_type);
    }
  ...
  void checkgrads(const vector<Example>& examples, const vector<Example>& base_examples, int iter, const int corpus_type) {
    UniLayer<xpu>& _olayer_linear=(1==corpus_type?_c1_olayer_linear:_c2_olayer_linear);
    MMCRFLoss<xpu>& _crf_layer=(1==corpus_type?_c1_crf_layer:_c2_crf_layer);
